 ...
 */
public class WaveformTrebleView extends View {
    ...
    public WaveformTrebleView(Context context) {
        super(context);
        init(context, null, 0);
    }

    public WaveformTrebleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public WaveformTrebleView(Context context, AttributeSet attrs, int defStyle) {
    ...
    void drawRecordingWaveform(short[] buffer, float[] waveformPoints) {    //difference between these arguments??? CRUCIAL!!!!!!
        ...
        for(int i=1; i < 4*width; i+=8)
            waveformPoints[i] = -Math.abs(yPoints[i/8 + 8*width/77]) + centerY;        //NO DOUBLING (full enough)

 ...
 */
public class WaveformMidView extends View {
    ...
    public WaveformMidView(Context context) {
        super(context);
        init(context, null, 0);
    }

    public WaveformMidView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public WaveformMidView(Context context, AttributeSet attrs, int defStyle) {
    ...
    void drawRecordingWaveform(short[] buffer, float[] waveformPoints) {    //difference between these arguments??? CRUCIAL!!!!!!
        ...
        //300-2400Hz <===== Consider shifting lower range up (325/350) && upper range down (2400)

        for(int i=1; i < 4*width; i+=22)   //20: 2600Hz (Treble shift: 6*width/53)
            waveformPoints[i] = -Math.abs(yPoints[i/44 + width/76]) + centerY;     //DOUBLING UP (to make fuller)

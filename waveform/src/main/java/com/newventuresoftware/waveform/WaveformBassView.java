 ...
 */
public class WaveformBassView extends View {
    ...
    public WaveformBassView(Context context) {
        super(context);
        init(context, null, 0);
    }

    public WaveformBassView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public WaveformBassView(Context context, AttributeSet attrs, int defStyle) {
    ...
    void drawRecordingWaveform(short[] buffer, float[] waveformPoints) {    //difference between these arguments??? CRUCIAL!!!!!!
        ...
        for(int i=1; i < 4*width; i+=70)                      //2*width: cuts off right half (all freqs above 350Hz)
            waveformPoints[i] = -Math.abs(yPoints[i/350 + width/500]) + centerY;      //QUINTUPLED UP (to make fuller)
